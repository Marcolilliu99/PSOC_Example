...
class SerialWorker(QRunnable):
    ...
    @pyqtSlot() #decoratore
    def run(self): #il run è l'unico metodo che va obbligatoriamente chiamato così perchè quando viene inizializzato un thread parallelo all'interno della main iwndow
        #viene lanciato il metodo run. Tutto quello che vogliamo che venga fatto dal thread parallelo deve stare qua dentro. Non esce da qua finchè non ha finito di fare tutto e poi dopo
        ...
        if not CONN_STATUS: #se niente è connesso
            ...
            except serial.SerialException: #se non riesce ad aprire la porta viene generaato l'errore SerialException
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        """
        # define worker
        self.serial_worker = SerialWorker(None) #stiamo inizializzando vuoto il nostro thread parallelo, non so ancora qual è il nome della porta

        super(MainWindow, self).__init__()

        # title and geometry
        self.setWindowTitle("ACCENSIONE LED")
        width = 400
        height = 320
    ...
    def initUI(self):
        ...
        button_hlay.addWidget(self.LEDON)
        button_hlay.addWidget(self.LEDOFF)
    ...
    def serialscan(self):
        ...
        self.LEDON = QPushButton(
            text="DISABILITATO",
            clicked=self.sendON

        )
        self.LEDON.setDisabled(True) 
        self.LEDOFF = QPushButton(
            text="DISABILITATO",
            clicked=self.sendOFF

        )
        self.LEDOFF.setDisabled(True) 
    ...
    #ora che abbiamo definitio i segnali dobbiamo creare le funzioni che permettano di collegarli tra di loro, il metodo chiave è ontoggle
    ##################
    # SERIAL SIGNALS #
    ##################
...
#stato del pulsante devo fare due cose diverse. Quindi serve una variabile booleana che è checked-->il segnale porta quell'informazione
    ...
    def on_toggle(self, checked): 
        ...
        if checked: #se il pulsante è stato premuto mi devo connettere alla porta, ma questo lo voglio fare nel thread parallelo, non nel main thread.
            ...
            self.LEDON.setDisabled(False) 
            self.LEDON.setText("ACCENDI") 
            self.LEDOFF.setDisabled(False) 
            self.LEDOFF.setText("SPEGNI") 
            ...
            self.LEDON.setDisabled(True) 
            self.LEDOFF.setDisabled(True) 
            self.LEDON.setText("DISABILITATO")
            self.LEDOFF.setText("DISABILITATO")
    def sendON(self):
        self.serial_worker.send("b")
        self.LEDON.setText("LED ON")
        self.LEDON.setDisabled(True) 
        self.LEDOFF.setDisabled(False) 

    def sendOFF(self):
        self.serial_worker.send("s")
        self.LEDOFF.setText("LED OFF")
        self.LEDOFF.setDisabled(True) 
        self.LEDON.setDisabled(False) 

    def check_serialport_status(self, port_name, status): #va a vedere se è riuscito o non a connettersi
        """!
