...
void INT_BUTTON_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)INT_BUTTON__INTC_NUMBER] = address;
}
...
cyisraddress INT_BUTTON_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)INT_BUTTON__INTC_NUMBER];
}
...
{
    *INT_BUTTON_INTC_PRIOR = priority << 5;
}
...
uint8 INT_BUTTON_GetPriority(void)
{
    ...
    priority = *INT_BUTTON_INTC_PRIOR >> 5;
